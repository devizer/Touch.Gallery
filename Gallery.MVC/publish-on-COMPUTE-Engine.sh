...
if [ ! -f "$target/.done" ]; then
  # OPTIONAL BUILD
    ...
    rm -rf .git
    cd Gallery.MVC
    dotnet publish -c Release -r linux-x64 --self-contained -o ../../Touch.Gallery-bin | tee $HOME/BUILD-Touch.Gallery.log
    popd
    rm -rf Touch.Gallery
...
(nohup bash -c './Gallery.MVC' | sudo tee $HOME/RUN-Touch.Gallery.log 2>&1) &
