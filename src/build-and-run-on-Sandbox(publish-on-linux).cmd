...
del "%TEMP%\%Snapshot%"
plink %SANDBOX_USER%@%SANDBOX_HOST% -l %SANDBOX_USER% -pw %SANDBOX_PASSWORD% -P %SANDBOX_PORT% "cd /%PROJECT% && 7z x -y %Snapshot% && rm -f %Snapshot% && dotnet publish -v:m -c Debug -r linux-x64 -o output; cd output; ./%PROJECT%"
